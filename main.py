...
try:
    ...
    logger.warning("Failed loading locations: %s", e)
    ALL_LOCATIONS = []
...
def generate_share_image(actual_lat, actual_lon, guessed_lat, guessed_lon, round_score, distance_km, filename=None):
    ...
    try:
        ...
        logger.warning("Error generating map: %s", e)
        return None
...
def index():
    ...
    session["actual_lat"], session["actual_lon"] = loc.get("lat"), loc.get("lon")
    ...
    return render_template("index.html",
                           lat=loc.get("lat"), lon=loc.get("lon"), heading=loc.get("heading", 0),
                           api_key=GOOGLE_API_KEY, round=round_num, score=score,
...
def guess():
    guessed_lat, guessed_lon = safe_float(request.form.get("lat")), safe_float(request.form.get("lon"))
    actual_lat, actual_lon = session.get("actual_lat"), session.get("actual_lon")
    if actual_lat is None or actual_lon is None: return redirect(url_for("index"))
    round_num, score = session.get("round", 1), session.get("score", 0)
    ...
    results.append({
        "round": round_num, "bar": bar,
        "distance_km": distance_km,
        "distance_mi": round(distance_km * 0.621371, 1),
        "round_score": round_score,
        "guessed_lat": guessed_lat, "guessed_lon": guessed_lon,
        "actual_lat": actual_lat, "actual_lon": actual_lon
    })
    session["results"], session["score"], session["round"] = results, score, round_num + 1
...
def round_result():
    ...
    share_image_url = generate_share_image(
        actual_lat=last_result["actual_lat"], actual_lon=last_result["actual_lon"],
        guessed_lat=last_result["guessed_lat"], guessed_lon=last_result["guessed_lon"],
        round_score=last_result["round_score"], distance_km=last_result["distance_km"]
    )
    return render_template("round_result.html",
                           result=last_result, score=score, round=last_result["round"],
                           api_key=GOOGLE_API_KEY, share_image_url=share_image_url
                           )
...
def freeplay():
    ...
    session["freeplay_actual_lat"], session["freeplay_actual_lon"] = loc["lat"], loc["lon"]
    session["freeplay_heading"] = loc.get("heading", 0)
    return render_template("freeplay.html", lat=loc["lat"], lon=loc["lon"], heading=loc.get("heading", 0),
...
def freeplay_guess():
    guessed_lat, guessed_lon = safe_float(request.form.get("lat")), safe_float(request.form.get("lon"))
    actual_lat, actual_lon = session.get("freeplay_actual_lat"), session.get("freeplay_actual_lon")
    ...
    if distance_km < 5:
        ...
        bar = "📍🟥📍"
    share_image_url = generate_share_image(actual_lat, actual_lon, guessed_lat, guessed_lon,
                                           max(0, int(1000 - distance_km)), distance_km)
    return render_template("freeplay_result.html",
                           guessed_lat=guessed_lat, guessed_lon=guessed_lon,
                           actual_lat=actual_lat, actual_lon=actual_lon,
                           distance_km=distance_km, distance_mi=distance_mi,
                           bar=bar, api_key=GOOGLE_API_KEY, share_image_url=share_image_url
                           )
...
def robots_txt():
    lines = ["User-Agent: *", "Disallow:", "Sitemap: https://sightcr.com/sitemap.xml"]
